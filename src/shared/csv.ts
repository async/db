...
export function parseCsvRecords(text: string, filePath = 'CSV file'): Array<Record<string, CsvRecordValue>> {
  ...
  return rows
    ...
    .map((row) => {
      const record: Record<string, CsvRecordValue> = {};
...
export function parseCsvRows(text: string): CsvCell[][] {
  const rows: CsvCell[][] = [];
  let row: CsvCell[] = [];
  let field = '';
  let quoted = false;
...
function fieldNameFromHeader(header: string, index: number): string {
...
function coerceCsvValue(cell: CsvCell, headerName: string): CsvRecordValue | undefined {
...
function isStringLikeHeader(headerName: string): boolean {

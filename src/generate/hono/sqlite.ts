...
export function renderSqliteAdapter(project: HonoSqliteProject, options: HonoSqliteOptions): string {
...
export function renderInitialMigration(resources: HonoResource[]): string {
...
function renderMigrationExecLines(resources: HonoResource[]): string {
  const sql = renderInitialMigration(resources).split('\n').filter((line) => !line.startsWith('--')).join('\n');
  return `  db.exec(${JSON.stringify(sql)});`;
}

function createTableSql(resource: HonoResource): string {
...
function sqliteTypeForField(field: HonoField): string {
  switch (field.type) {
    case 'number':
      return 'REAL';
    case 'boolean':

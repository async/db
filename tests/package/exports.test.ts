...
test('package metadata exposes @async/db with the async-db CLI', async () => {
  ...
  assert.equal(packageJson.devDependencies['@async/pipeline'], '0.9.35');
...
test('generated async-pipeline workflow owns release, package preview, snapshot, and Pages lifecycle jobs', async () => {
  ...
  assert.doesNotMatch(workflow, /pnpm async-pipeline run publish-github/);
  assert.doesNotMatch(workflow, /pnpm async-pipeline run publish/);
  assert.doesNotMatch(workflow, /pnpm async-pipeline run release-doctor/);

...
import { openDb as typedOpenDb, syncDb as typedSyncDb, loadConfig as typedLoadConfig } from '../../src/index.js';
...
test('data-first fixtures generate schema, types, and runtime state', async () => {
  ...
  assert.match(await readFile(path.join(cwd, '.db/types/index.d.ts'), 'utf8'), /export type User =/);
...
test('outputs config writes committed sync artifacts', async () => {
  ...
  assert.match(await readFile(path.join(cwd, 'src/generated/db.types.d.ts'), 'utf8'), /export type User =/);
...
test('resource aliases that collapse across camelCase and kebab-case fail fast', async () => {
  ...
  const config = await loadConfig({ cwd });
  const rejectsWithAliasCollision = (error: any) => {
...
test('generated enum union aliases use stable inline and multiline formatting', async () => {
  ...
  const typesPath = path.join(cwd, '.db/types/index.d.ts');
...
test('types.commitOutFile writes a committed type copy', async () => {
  ...
  const ignoredTypes = await readFile(path.join(cwd, '.db/types/index.d.ts'), 'utf8');
...
test('explicit .ts type output paths stay supported for compatibility', async () => {
  const cwd = await makeProject();
  await writeConfig(cwd, `export default {
    types: {
  ...
  assert.match(await readFile(path.join(cwd, 'src/generated/db.types.ts'), 'utf8'), /users: User;/);
});
